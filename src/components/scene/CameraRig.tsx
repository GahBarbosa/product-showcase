...
import { usePresentationStore, type PresentationPhase } from "../../app/presentationStore";
...
const CameraRig = forwardRef<CameraControllerHandle, { phase: PresentationPhase }>(({ phase }, ref) => {
  ...
  const markInteraction = usePresentationStore((state) => state.markInteraction);
  ...
  return <OrbitControls ref={controls} enabled={phase === "explore"} onStart={markInteraction} enablePan={false} minDistance={0.65} maxDistance={2.2} minPolarAngle={Math.PI / 4} maxPolarAngle={(Math.PI * 3) / 4} />;
});
